...
$arPrices = array();
$arStickers = array();
$arSlidersForBlocks = array();
foreach ($arResult['ITEMS'] as $key => &$arItem) {
	if (in_array($arItem['CODE'], $arMarketPrices)) {
		$arItem['PRICE'] = true;
	}

	if (!$arItem['PRICE'] && $arItem['PROPERTY_TYPE'] == 'N' && ($arItem['DISPLAY_TYPE'] == 'A' || $arItem['DISPLAY_TYPE'] == 'B') && empty($arItem['VALUES']['MAX']['HTML_VALUE']) && empty($arItem['VALUES']['MIN']['HTML_VALUE']) && !empty($arItem['VALUES']['MIN']['VALUE']) && !empty($arItem['VALUES']['MAX']['VALUE'])){
        $arSlidersForBlocks[$key] = $arItem;
    }
	...
	if (in_array($arItem['CODE'], $arParams['STICKERS']) && !empty($arItem['VALUES'])) {
        ...
        CRZBitronic2CatalogUtils::checkActiveValuesInFilter($arItem,$arResult);
		...
		if ($code == 'CUSTOM') {
			foreach ($arItem['VALUES'] as $arValue) {
				...
				$arValue['CLASS'] = str_replace('%20', ' ', $arValue['URL_ID']);
   ...
   CRZBitronic2CatalogUtils::checkActiveValuesInFilter($arItem,$arResult);
	...
	if (strpos($arItem['CODE'], 'RZ_AVAILABLE') === 0) {
	    foreach ($arItem['VALUES'] as $arValue){
	        if ($arValue['CHECKED']){
	            $arResult['DATA_FILTER']['RZ_AVAILABLE'][] = $arValue['VALUE'];
            }
...
if (!empty($arSlidersForBlocks)){
    CRZBitronic2CatalogUtils::setDisabledValForSlidersInFilter($arResult['ITEMS'],$arSlidersForBlocks,$GLOBALS[$arParams['FILTER_NAME']],$arParams);
}
...
if (!empty($arResult['DATA_FILTER'])){
    $this->__component->__parent->arResult['DATA_OF_FILTER'] = $arResult['DATA_FILTER'];
    unset($arResult['DATA_FILTER']);
}