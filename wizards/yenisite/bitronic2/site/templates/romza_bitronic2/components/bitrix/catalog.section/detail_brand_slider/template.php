...
?>
<?if (!empty($arResult['SECTIONS_WITH_ELEMENTS'])):?>
    <div class="sub-categories wow fadeInUp columns">
        <?foreach ($arResult['SECTIONS_WITH_ELEMENTS'] as $arSection):?>
                <a href="<?=$arSection['HREF']?>" class="link" title="<?=$arSection['HEADER']?>">
                    <span class="text"><?=$arSection['HEADER']?></span>
...
<?endif?>
<?foreach ($arResult['SECTIONS_WITH_ELEMENTS'] as $arSection):?>
<div class="similar-products <? if($arParams['STICKER'] == 'WITH_GIFT'): ?> gift-slider<? endif ?><?= $bHoverMode ? ' __expanded' : '' ?>">
	<header>
        <a href="<?=$arSection['HREF']?>" class="link-bd link-std"><?=$arSection['HEADER']?></a>
        <sup><?=$arSection['ELEMENT_CNT']?></sup>
    </header>
