...
<html lang="<?= LANGUAGE_ID ?>">
    <head>
        ...
        Mobile::Init();
        $bEditeMode = Tools::isEditModeOn();
        $bShowMobileHrefForTelephone = !Tools::isEditModeOn() && $rz_b2_options['mobile-phone-action'] != 'callback' && Mobile::isMobile();
        ...
        <?//!!!SET CATALOG PARAMS FOR HIDE ITEMS
           CRZBitronic2CatalogUtils::setParamsForHideItemsInCatalog();
        ...
        <? //!!!SET CAPTCHA FOR REGISTRATION
            CRZBitronic2CatalogUtils::setCaptchaForRegistration();
        ?>

        <? //!!!SET CAPTCHA FOR FEEDBACK BLOG
            CRZBitronic2CatalogUtils::setCaptchaBlog();
        ?>
        <?//!!!CHECK IF SOCIAL ON
         $bIncludeWigetSliderScript = CRZBitronic2Settings::checkEnebleSocialServices();
         ?>
        <?
        ...
        if (!empty($rz_b2_options['site-font'])){
            $asset->addString('<link href="https://fonts.googleapis.com/css?family=' . $rz_b2_options['site-font'] . ':400,400i,700,700&subset=cyrillic" rel="stylesheet" id="site-font" property="stylesheet">');
        }
        ...
        $asset->addJs(SITE_TEMPLATE_PATH . '/js/3rd-party-libs/validator_custom.js');
        ...
        if ($bMainPage) {
            ...
            $asset->addJs(SITE_TEMPLATE_PATH . "/js/custom-scripts/inits/pages/initHomePage.js");
             ...
             if ('Y' == $rz_b2_options['block_home-reviews'] && !Tools::isEditModeOn()) {
                $asset->addJs(SITE_TEMPLATE_PATH . "/js/back-end/ajax/reviews.js");
            }
        }
        ...
        if ($USER->IsAdmin() && $rz_b2_options['top-line-position'] == 'fixed-top'){
            $asset->addJs(SITE_TEMPLATE_PATH . "/js/back-end/fix_bitrix_panel.js");
        }
        ...
        if ($rz_b2_options['block_show_oneclick'] === 'Y') {
            $asset->addJs(SITE_TEMPLATE_PATH . '/js/custom-scripts/inits/forms/initQuantityChange.js');
        }
        if ($APPLICATION->GetCurDir() == SITE_DIR && $bIncludeWigetSliderScript) {
            $asset->addJs(SITE_TEMPLATE_PATH . '/js/custom-scripts/inits/sliders/initWidgetSlider.js');
        }
        if ($rz_b2_options['use_google_captcha'] == 'Y') {
            $asset->addString('<script src="https://www.google.com/recaptcha/api.js" async defer></script>');
            $asset->addJs(SITE_TEMPLATE_PATH . '/js/custom-scripts/inits/forms/initGoogleCaptcha.js');
        }
        if ($rz_b2_options['block_show_viwed'] == 'Y' && !$bEditeMode){
            CJSCore::Init(array('rz_b2_bx_catalog_item'));
           $asset->addJs(SITE_TEMPLATE_PATH . '/js/back-end/ajax/viewed_products.js');
        }?>
...
<body
    ...
    <? //todo: additional price param ?>
        ...
        style="background: <?= $rz_b2_options['color-body'] ?>; <?=!empty($rz_b2_options['site-font']) ? 'font-family:'.$rz_b2_options['site-font'].', sans-serif;' : ''?>"<? //COLOR?>
...
<? endif ?>
    ...
    <button class="btn-main to-top <?=$rz_b2_options['btn-to-top']?>">
...
<? endif ?>
    ...
    <div id="top-line-backend" class="top-line">
        <div class="container">
            <div class="top-line-content clearfix">
                ...
                <? endif ?>
                <? if ($rz_b2_options['block_show_viwed'] == 'Y'): ?>
                    <?Tools::IncludeArea('header','viewed');?>
                <? endif ?>
    ...
    <header class="page-header" data-sitenav-type="<?= $rz_b2_options['header-version'] ?>"
            ...
            style="background: <?= $rz_b2_options['color-header'] ?>"><? //COLOR?>
            ...
            <div class="header-main-content <?= CRZBitronic2Settings::isPro() ? 'with-delivery' : '' ?>">
                ...
                <a href="<?= SITE_DIR ?>" class="brand">
                    ...
                    <div class="brand-desc"
                         itemprop="description"><? $APPLICATION->IncludeComponent("bitrix:main.include", "", array_merge($arDefIncludeParams, array("PATH" => SITE_DIR . "include_areas/header/logo_under_text.php")), false, array("HIDE_ICONS" => "N")); ?>
                    </div><!-- logo-text -->
                </a><!-- logo -->
                ...
                <div class="header-contacts">
                    <i id="switch-contacts" class="flaticon-phone12 phone" data-popup="^.header-contacts>.contacts-content">
                      <? if ($bShowMobileHrefForTelephone): ?>
