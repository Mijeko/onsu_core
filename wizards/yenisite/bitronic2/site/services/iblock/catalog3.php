...
if ($IBLOCK_OFFERS_ID)
{
	$iblockCodeOffers = "{$moduleCode}_offers";
	//IBlock fields
	$iblock = new CIBlock;
	$arFields = Array(
		"ACTIVE" => "Y",
		...
		"CODE" => "{$moduleCode}_offers",
		"XML_ID" => $iblockCodeOffers
	);
	$iblock->Update($IBLOCK_OFFERS_ID, $arFields);
}

//This will be executed only for newly imported iblock
if ($IBLOCK_CATALOG_ID)
{
	...
	$arFields = Array(
		...
		"CODE" => "{$moduleCode}_catalog",
		"XML_ID" => $iblockCode
	);
	$iblock->Update($IBLOCK_CATALOG_ID, $arFields);
	
	
	//SET MESSAGES CATALOG
	$arMessages = array(
		'ELEMENT_NAME' 		=> GetMessage('CATALOG-ELEMENT_NAME'),
		'ELEMENTS_NAME' 	=> GetMessage('CATALOG-ELEMENTS_NAME'),
		'ELEMENT_ADD' 		=> GetMessage('CATALOG-ELEMENT_ADD'),
		'ELEMENT_EDIT' 		=> GetMessage('CATALOG-ELEMENT_EDIT'),
		'ELEMENT_DELETE' 	=> GetMessage('CATALOG-ELEMENT_DELETE'),
		'SECTION_NAME' 		=> GetMessage('CATALOG-SECTION_NAME'),
		'SECTIONS_NAME' 	=> GetMessage('CATALOG-SECTIONS_NAME'),
		'SECTION_ADD' 		=> GetMessage('CATALOG-SECTION_ADD'),
		'SECTION_EDIT' 		=> GetMessage('CATALOG-SECTION_EDIT'),
		'SECTION_DELETE' 	=> GetMessage('CATALOG-SECTION_DELETE'),
		
	);
	CIBlock::SetMessages($IBLOCK_CATALOG_ID, $arMessages);
	unset($arMessages);

	if ($IBLOCK_OFFERS_ID && CModule::IncludeModule("catalog"))
	{
		...
		//SET MESSAGES OFFERS
		$arMessages = array(
			'ELEMENT_NAME' 		=> GetMessage('OFFERS-ELEMENT_NAME'),
			'ELEMENTS_NAME' 	=> GetMessage('OFFERS-ELEMENTS_NAME'),
			'ELEMENT_ADD' 		=> GetMessage('OFFERS-ELEMENT_ADD'),
			'ELEMENT_EDIT' 		=> GetMessage('OFFERS-ELEMENT_EDIT'),
			'ELEMENT_DELETE' 	=> GetMessage('OFFERS-ELEMENT_DELETE'),
			'SECTION_NAME' 		=> GetMessage('OFFERS-SECTION_NAME'),
			'SECTIONS_NAME' 	=> GetMessage('OFFERS-SECTIONS_NAME'),
			'SECTION_ADD' 		=> GetMessage('OFFERS-SECTION_ADD'),
			'SECTION_EDIT' 		=> GetMessage('OFFERS-SECTION_EDIT'),
			'SECTION_DELETE' 	=> GetMessage('OFFERS-SECTION_DELETE'),
			
		);
		CIBlock::SetMessages($IBLOCK_OFFERS_ID, $arMessages);
	}
...
//demo weight
	if (CModule::IncludeModule("catalog")) {
		$rs = CIBlockElement::GetList(array(), array("IBLOCK_ID" => $IBLOCK_CATALOG_ID), false, false, array("ID"));
		while($ar = $rs->GetNext())
		{
			CCatalogProduct::Update($ar['ID'], array("WEIGHT"=>rand(100, 1000)));
		}
	}
...
if ($IBLOCK_CATALOG_ID) {
	//This should be executed for both new and old catalog iblock
	$arMacros = array("CATALOG_IBLOCK_ID" => $IBLOCK_CATALOG_ID);
	
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/catalog/index.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/.catalog.menu_ext.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include_areas/header/compare.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include_areas/index/main_spec.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include_areas/index/cool-slider.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include_areas/index/categories.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include_areas/index/brands.php", $arMacros);
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/pricelist/index.php", $arMacros);

	//============= LITE ONLY =============
	if (CModule::IncludeModule('yenisite.market')) {
		CMarketCatalog::Add($IBLOCK_CATALOG_ID, (WIZARD_INSTALL_DEMO_DATA ? 1 : 0));
	}
}
?>