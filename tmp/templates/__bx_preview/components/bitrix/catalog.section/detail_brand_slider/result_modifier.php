...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        $arResult['SECTIONS_WITH_ELEMENTS'][$arItem['IBLOCK_SECTION_ID']]['ITEMS'][] = $arItem;
    ...
    $rsSections = CIBlockSection::GetList(
        array(),
        array('IBLOCK_ID'=> $arParams['IBLOCK_ID'], 'ID' => $arSectionsID, 'ACTIVE' => 'Y', 'ELEMENT_SUBSECTIONS' => 'Y', 'CNT_ACTIVE' => 'Y', 'PROPERTY' => array($arParams['BRAND_PROP_CODE'] => $arParams['BRAND_PROP_VALUE'])),
        true,
        array()
    );
    unset($arSectionsID);
    ...
    if ($bSefMode){
        $rsSections->SetUrlTemplates($arParams["SEF_FILTER"]);
    }
    while ($arSections = $rsSections->GetNext()){
        ...
        if (!$bSefMode) {
            ...
            $smartFilter->arResult['ITEMS'] = array($arFilterProp['ID'] => $arFilterProp);
            $arResult['SECTIONS_WITH_ELEMENTS'][$arSections['ID']]['HREF'] = $smartFilter->makeSmartUrl($arSections["DETAIL_PAGE_URL"], true);
        }
