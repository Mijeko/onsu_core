...
if ($normalCount > 0):?>
    <!-- sale.basket.basket/big_basket/basket_items -->
	<div id="basket-big" class="basket-big tab-target">
		<table class="items-table" id="basket_items">
			...
			<tr>
				...
				<? endif; if ($arResult['COLUMNS']['ACTIONS']):?><th></th>
                <? endif ?>
			</tr>
			</thead>
			<tfoot>
					...
					<div class="totals">
						<table class="table_totals">
							...
							<?if ($arParams["PRICE_VAT_SHOW_VALUE"] == "Y"):?>
								...
								-->
                                <tr>
                                    <td class="text">Стоимость доставки</td>
                                    <td class="value"><?= $arResult["allSum"] < 500 ? 150 : 0 ?> <span class="b-rub">Р</span></td>
                                </tr>
							<?endif;?>
                            <? if ($arResult["allSum"] < 500){
                                $arResult["allSum"] = $arResult["allSum"] + 150;
                            } ?>
							...
							<tr>
                                <td class="text"><?=GetMessage("BITRONIC2_SALE_TOTAL_DELIVERY", array('#DELIVERY_URL#' => $arParams['DELIVERY_URL']))?></td>
				...
				if ($arItem["DELAY"] == "N" && !isset($arItem["NOT_AVAILABLE"]) && $arItem["NOT_AVAILABLE"] != true):// && $arItem["CAN_BUY"] == "Y"):
					...
					<? if ($arResult['COLUMNS']['NAME']): ?>
						...
						<? if ($bShowName): ?>
							...
							<? endif ?>
								<span class="text"><?= $arItem['PREVIEW_TEXT'] ? : $arItem['NAME'] ?></span>
							<? if (!empty($arItem['DETAIL_PAGE_URL'])): ?>
							</a>
							<? endif ?>
						<? endif ?>
					...
					<? if ($arResult['COLUMNS']['QUANTITY']): ?>
						...
						<span class="quantity-counter basket_quantity_control"
							...
							//todo griff: "Минимальный заказ" сделать тоже самое для маленькой корзины
							$max = isset($arItem["AVAILABLE_QUANTITY"]) ? "max=\"".$arItem["AVAILABLE_QUANTITY"]."\"" : "";
							$useFloatQuantity = ($arParams["QUANTITY_FLOAT"] == "Y") ? true : false;
							$useFloatQuantityJS = ($useFloatQuantity ? "true" : "false");
							?>

							<button type="button" id="QUANTITY_DOWN_<?=$arItem['ID']?>"
								class="btn-silver quantity-change decrease<?=($arItem['QUANTITY'] <= $min ? ' disabled' : '')?>"
								data-ratio="<?= $ratio ?>"
								>
							...
							<input
								...
								value="<?= $arItem["QUANTITY"] < $min ? $min : $arItem["QUANTITY"] ?>"
					...
					<? if ($arResult['COLUMNS']['PRICE']): ?>
						<td class="price">
							...
							<span class="price-new"><?=CRZBitronic2CatalogUtils::getElementPriceFormat($arItem['CURRENCY'], $arItem["PRICE"], $arResult["PRICE_FORMATED"], array('ID'=> 'current_price_'.$arItem["ID"]))?>/<?= $arItem['MEASURE_NAME']?></span>
							<div>
