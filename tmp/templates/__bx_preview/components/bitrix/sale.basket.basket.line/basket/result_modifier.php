...
if(!empty($arResult["CATEGORIES"]["READY"]))
{
	...
	foreach($arResult["CATEGORIES"]["READY"] as $key=>$item){
        ...
        //Получаем текущий элемент из инфоблока и достаем его "привильное" название из "PREVIEW_TEXT"
        $element = CIBlockElement::GetByID($item['PRODUCT_ID'])->GetNext();
        $arResult["CATEGORIES"]["READY"][$key]['PREVIEW_TEXT'] = $element['PREVIEW_TEXT'];
        
        //Получаем минимальный возможный заказ и кратность
        $element_min = CIBlockElement::GetProperty($element['IBLOCK_ID'],$element['ID'], ['sort' => 'asc'], ['CODE' => 'MINIMALNYY_ZAKAZ'])->GetNext();
        $element_krat = CIBlockElement::GetProperty($element['IBLOCK_ID'],$element['ID'], ['sort' => 'asc'], ['CODE' => 'KRATNOST'])->GetNext();
        $arResult["CATEGORIES"]["READY"][$key]['MIN_ZAK'] = $element_min['VALUE'];
        $arResult["CATEGORIES"]["READY"][$key]['KRAT'] = $element_krat['VALUE'];
        
        //echo "<pre style='text-align:left;'>";print_r($element);echo "</pre>";
	    if($bConvertCurrency)
		{
...
<?
if (empty($arResult['CATEGORIES']['READY']) && !empty($arResult['CATEGORIES']['DELAY'])){
    $arResult["TOTAL_COUNT"] = count($arResult['CATEGORIES']['DELAY']);
    $arResult["GO_TO_BASKET"] = true;
}
?>