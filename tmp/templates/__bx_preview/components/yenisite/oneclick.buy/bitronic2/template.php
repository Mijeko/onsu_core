	...
	<? if ($arParams['FIELD_QUANTITY'] == 'Y' && $_REQUEST['RZ_BASKET'] !== 'Y'): ?>
		<div>
			...
			<label class="textinput-wrapper name">
				...
				<input type="number" class="textinput" name="QUANTITY"
                       value="<?= $arResult['MIN_ZAK'] ? : 1 ?>"
                       min="<?= $arResult['MIN_ZAK'] ? : 1 ?>"
                       step="<?= $arResult['KRAT'] ? : 1 ?>"
                       title=""/>
			</label>
		</div>
	<? endif ?>
	...
	<? if ('Y' == $arResult['USE_CAPTCHA'] && strlen($arResult["CAPTCHA_CODE"]) > 0): ?>
		<div>
			...
			<div class="modal-captcha-wrap">
                ...
                <div style="font-size: smaller">Для смены картинки щелкните по ней.</div>
			</div>
