...
$id = "voting_current_".$this->randString();
...
?><div id="<?=$id?>_form"><?$APPLICATION->IncludeComponent(
		...
		Array(
            ...
            'USE_GOOGLE_CAPTCHA' => $rz_b2_options['use_google_captcha'] == 'Y'
		),
		...
		<span class="vote-form-box-button vote-form-box-button-single">
			<a name="show_form" <?
				?>onclick="BX('<?=$id?>_form').style.display='block';BX('<?=$id?>_result').style.display='none';return false;" <?
					?>href="<?=htmlspecialcharsbx($APPLICATION->GetCurPageParam("", array("VOTE_ID","VOTING_OK","VOTE_SUCCESSFULL", "view_result")))?>" <?
