...
?>
		<div class="title-h2"><?= $arParams['ADDRESS_STR'] ?></div>
		<?include $_SERVER["DOCUMENT_ROOT"].SITE_TEMPLATE_PATH.'/include/debug_info_dynamic.php';?>
		<?
		$arParams['MAP_WIDTH'] = '100%';
		$arParams['MAP_HEIGHT'] = 'auto';
		$style = '';
		if(intval($arParams['MAP_WIDTH']) > 0) {
			$style = 'style="width:' . $arParams['MAP_WIDTH'] . 'px;';
		} else {
			$style = 'style="width: 100%;';
		}
		if(intval($arParams['MAP_HEIGHT']) > 0) {
			$style .= 'height:' . $arParams['MAP_WIDTH'] .'px;';
		} else {
			$style .= 'height:auto;';
		}
		if(strlen($style) > 0) {
		...
		?>
		<div class="img-container limiting map" <?=$style?>>
			<?
