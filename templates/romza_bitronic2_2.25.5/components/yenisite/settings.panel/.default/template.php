...
<? if ($firstHit == 0): ?>
	...
	<script>
		$(document).on('click','#settings-toggle',function(e){
			RZB2.utils.setCookie('yns_first_hit', 1,'rz_');
		});
	</script>
<? endif ?>
<?$frame->end();
					...
					<div class="combo-target" id="settings_<?=$groupID?>">
						...
						<div class="combo-target-content">
							...
							<? foreach ($arItem['SETTINGS'] as $setting): ?>
								<? if(isset($arResult['SETTINGS'][$setting]['hidden']) && $arResult['SETTINGS'][$setting]['hidden'] == true && $arResult['SETTINGS'][$setting]['force-show'] !== true) {
									continue;
								}
