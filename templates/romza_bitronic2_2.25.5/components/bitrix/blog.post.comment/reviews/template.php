	...
	if($arResult["imageUploadFrame"] != "Y")
	{
        ...
        <i class="flaticon-check14"></i><?=GetMessage('BITRONIC2_I_ACCEPT')?>
					...
					<div class="form-footer">
						...
						?>
                        <?if ($arResult["use_captcha"]!==true && $arParams['USE_GOOGLE_CAPTCHA'] == 'Y'):?>
                            <div class="google-captcha-ni-review">
                                <?CRZBitronic2CatalogUtils::showGoogleCaptcha()?>
                            </div>
                        <?endif?>
						<button class="btn-main" value="<?=GetMessage("B_B_MS_SEND")?>" type="submit" name="sub-post" id="post-button"><span class="btn-text"><?=GetMessage("B_B_MS_SEND")?></span></button>
					</div>
