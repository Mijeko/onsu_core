...
if (!empty($arResult["DELIVERY"])) {
    ...
    ?>
    <div class="title-h3"><?= GetMessage("BITRONIC2_SOA_TEMPL_DELIVERY") ?></div>
    ...
    foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery) {
        ...
        <label class="col-md-2 col-sm-3 col-xs-6" for="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>">
            ...
            <span class="radio-item">
                ...
                <span itemscope itemtype="http://schema.org/ImageObject" class="radio-img" <?= $clickHandler ?>>
							<img itemprop="contentUrl" src="<?= $deliveryImgURL ?>" alt="<?= $arDelivery["NAME"] ?>">
						</span>

						<span class="radio-item-header" <?= $clickHandler ?>><?= htmlspecialcharsbx($arDelivery["NAME"]) ?></span>

						<span class="bx_result_price">
							<? if (isset($arDelivery["PRICE"])) {
                                echo GetMessage("BITRONIC2_SALE_DELIV_PRICE") . ": <b>";
                                if (isset($arDelivery['DELIVERY_DISCOUNT_PRICE'])
                                    && round($arDelivery['DELIVERY_DISCOUNT_PRICE'], 2) != round($arDelivery["PRICE"], 2)
                                ) {
                                ...
                                if (strlen($arDelivery["PERIOD_TEXT"]) > 0) {
                                    echo GetMessage('BITRONIC2_SALE_TRANSIT') . ": <b>" . $arDelivery["PERIOD_TEXT"] . "</b>";
                                    echo '<br />';
                                }
                                if ($arDelivery["PACKS_COUNT"] > 1) {
                                    echo '<br />';
                                    echo GetMessage('BITRONIC2_SALE_PACKS_COUNT') . ': <b>' . $arDelivery["PACKS_COUNT"] . '</b>';
                                }
                            } elseif (isset($arDelivery["CALCULATE_ERRORS"])) {
                                CRZBitronic2CatalogUtils::ShowMessage(array("MESSAGE" => $arDelivery["CALCULATE_ERRORS"], "TYPE" => "ERROR"));
                            } else {
						...
						<span class="radio-item-description" <?= $clickHandler ?>>
							<?
                            ...
                            if (count($arDelivery["STORE"]) > 0 && $arDelivery['CHECKED'] == 'Y'):
                                ...
                                <span id="select_store"<? if (strlen($arResult["STORE_LIST"][$arResult["BUYER_STORE"]]["TITLE"]) <= 0) echo " style=\"display:none;\""; ?>>
									<span class="select_store"><?= GetMessage('BITRONIC2_SOA_ORDER_GIVE_TITLE'); ?>
                                        : </span>
                    ...
                    <tr>
                        ...
                        <td rowspan="2" class="price">
                            ...
                            if ($price = $extraService->getPrice()) {
                                echo GetMessage('BITRONIC2_SOA_TEMPL_SUM_PRICE') . ': ';
