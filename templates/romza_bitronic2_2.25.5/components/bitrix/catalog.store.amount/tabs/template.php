...
if(isset($arResult["IS_SKU"]) && $arResult["IS_SKU"] == 1 && isset($arResult["JS"]["SKU"])):
	foreach ($arResult["JS"]["SKU"] as $keyOffer => $arJSOffer)
	{
		...
		ob_start();
			...
			{
                if ($arParams['SHOW_EMPTY_STORE'] == 'N' && empty($arJSOffer[$arItem['ID']]) && $arJSOffer[$arItem['ID']] <= 0){
                    if (!$notEmpty) {
                        $empty = true;
                    } else {
                        $empty = false;
				...
				?>
                <div class="store" data-popup="^.store>.popup_map">
                    <div class="store-info">
                        <?if (!empty($arItem['TITLE'])):?>
                            <div class="store-name"><?=$arItem['TITLE']?></div>
                        <?endif?>
                        <?if (!empty($arItem['ADDRESS'])):?>
                            <div class="store-address"><?=$arItem['ADDRESS']?></div>
                        <?endif?>
                        <?if (!empty($arItem['PHONE'])):?>
                            <div class="store-phone"><?=$arItem['PHONE']?></div>
                        <?endif?>
                    </div>
                    <div class="store-count">
                        ...
                        case 'graphic':?>
                            ...
                            <?break;
                        case 'numeric':?>
                            <span class="store-numeric"><?= (int)$arJSOffer[$arItem['ID']] ?> <?=$arParams['MEASURE_NAME']?></span>
                            <?break;
                        default:
                        case 'text':?>
	...
	<?unset($arResult["STORES"]);
		...
		<?if(!empty($arResult["STORES"])):?>
			<?foreach($arResult["STORES"] as $pid => $arItem):?>
        <? if ($arParams['SHOW_EMPTY_STORE'] == 'N' && empty($arItem['REAL_AMOUNT']) && $arItem['REAL_AMOUNT'] <= 0){
            if (!$notEmpty) {
                $empty = true;
            } else {
                $empty = false;
                ...
                <div class="store" data-popup="^.store>.popup_map">
                    ...
                    <div class="store-count">
                        <?switch($arParams['STORE_DISPLAY_TYPE']):
                            ...
                            <?break;
                            case 'numeric':?>
                                <span class="store-numeric"><?= (int)$arItem["AMOUNT"] ?> <?=$arParams['MEASURE_NAME']?></span>
                            <?break;
                            default:
                            case 'text':?>
