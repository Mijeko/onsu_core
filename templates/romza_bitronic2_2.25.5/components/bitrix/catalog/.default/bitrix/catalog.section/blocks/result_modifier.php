...
$bHoverPicture = $arParams['SHOW_HOVER_PHOTO'];
if (!$boolConvert)
...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $index => &$arItem) {
        ...
        if ($arParams['HIDE_ICON_SLIDER'] != 'Y' && !$arParams['IS_MOBILE']) {
            ...
            if ($arParams['SHOW_HOVER_PHOTO']){
                CRZBitronic2CatalogUtils::getSliderImages($arItem,$arItem['VIP'] ? $arParams['RESIZER_SECTION_VIP'] : $arParams['RESIZER_SECTION']);
            }
        ...
        if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            ...
            if ('Y' == $arParams['PRODUCT_DISPLAY_MODE_CUSTOM'] && $arParams['SHOW_BUY_BTN']) {
                ...
                foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                    ...
                    if ($bHoverPicture){
                        if (empty($arOffer['MORE_PHOTO'])) {
                            CRZBitronic2CatalogUtils::getSliderImages($arOffer, $arItem['VIP'] ? $arParams['RESIZER_SECTION_VIP'] : $arParams['RESIZER_SECTION']);
                        }
                        $arPicture = next($arOffer['MORE_PHOTO']);
                        if (empty($arPicture) && $arParams['ADD_PARENT_PHOTO'] == 'Y'){
                            $arOffer['SECOND_PICTURE_PRINT']['SRC'] = $arItem['PICTURE_PRINT']['SRC'];
                        } else {
                            $arOffer['SECOND_PICTURE_PRINT']['SRC'] = $arPicture['SRC'];
                        }
                    }
                ...
                foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                    ...
                    if ($bHoverPicture){
                        $arOneRow['HOVER_PICTURE'] = $arOffer['SECOND_PICTURE_PRINT'];
                    }
        ...
        if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            ...
            if (empty($arItem['MORE_PHOTO'])) {
                if (!isset($arItem['OFFERS_SELECTED'])) {
                    $intSelected = -1;
                    $intMinPricedOffer = 0;
                    unset($minPriceOfOffer);
                    foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                        $minPriceOfOffer = $minPriceOfOffer ? $minPriceOfOffer : $arOffer['MIN_PRICE']['DISCOUNT_VALUE'];
                        if ($arItem['OFFER_ID_SELECTED'] > 0) {
                            $foundOffer = ($arItem['OFFER_ID_SELECTED'] == $arOffer['ID']);
                        } elseif ($minPriceOfOffer >= $arOffer['MIN_PRICE']['DISCOUNT_VALUE']) {
                            $minPriceOfOffer = $arOffer['MIN_PRICE']['DISCOUNT_VALUE'];
                            $intMinPricedOffer = $keyOffer;
                            $foundOffer = $arOffer['CAN_BUY'];
                        }
                    ...
                    $intSelected = $arItem['OFFERS_SELECTED'];
                }
