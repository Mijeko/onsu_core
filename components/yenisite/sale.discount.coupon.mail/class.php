...
class CSaleDiscountCouponMailComponent extends CBitrixComponent
{
	...
	{
		global $USER;
		//Если нужно полностью запретить генерацию промокодов для незарегистрированных пользователей:
		if ($this->arParams['UNREGISTERED_USER'] === "N")
            if (empty($USER->GetID())) return;
        
		//Проверяем количество заказов пользователя
        //if ((int)$this->arParams['COUNT_OF_ORDER'] > 0) {
        //    $orders = Order::getList(['filter' => ["USER_ID" => $USER->GetID()]]);
        //    if ($orders->getCount() < (int)$this->arParams['COUNT_OF_ORDER']) {
        //        return;
        //    }
        //}
		...
		switch ($this->arParams['COUPON_TYPE']){
            case 'Basket':
                $type = DiscountCouponTable::TYPE_BASKET_ROW;
                break;
            case 'Order':
		...
		{
			if (!empty($USER->GetID()))
		        $coupon = $USER->GetID() . "-" . DiscountCouponTable::generateCoupon(true);
			else
    ...
    /**
     * Если купон/промокод не привязан к акции то акция создается и возвращается её идентификатор
     * @return int
