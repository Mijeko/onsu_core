...
?>
<div <?if($arParams['SHOW_SIMILAR_ORG']):?>id="related-similar-price-to-<?=$arParams['ELEMENT_ID']?>" itemprop="isSimilarTo" itemscope itemtype="https://schema.org/Product"<?endif?> <?if ($arParams['DISPLAY_IS_RELATED']):?>id="related-gifts-to-<?=$arParams['ELEMENT_ID']?>" itemprop="isRelatedTo" itemscope itemtype="https://schema.org/Product"<?endif?> class="viewed-products detail-slider<? if($arParams['STICKER'] == 'WITH_GIFT'): ?> gift-slider<? endif ?><?= $bHoverMode ? ' __expanded' : '' ?> <?=$arParams['ORDER_DETAIL_BLOCKS'] ? "drag-section ".$arParams['NAME_ORDER_BLOCK'] : ''?>" <?=$arParams['ORDER_DETAIL_BLOCKS'] ? "data-order=\"{$arParams['ORDER_DETAIL_BLOCKS']}\"" : ''?>>
	<header><?=$arParams['~HEADER_TEXT']?></header>
		...
		<div class="scroll-slider frame">
			<div class="slider-content slidee catalog"><?
				foreach($arResult['ITEMS'] as $arItem):
					...
					$strObName = 'ob'.preg_replace("/[^a-zA-Z0-9_]/", "x", $strMainID).($arParams['SUFFIX'] ?: '_ds');
					$imgTitle = (
				...
				?><div class="slider-item<?=(!$arItem['CAN_BUY'] && !$arItem['ON_REQUEST'] ? ' out-of-stock' : '')?> catalog-item" id="<?=$arItemIDs['ID']?>">
					...
					<? if($arParams['STICKER'] == 'WITH_GIFT'): ?>

					<span class="stickers">
						<span class="sticker with-gift flaticon-6"><?=($arParams['TEXT_LABEL_GIFT']?:GetMessage('BITRONIC2_SLIDER_WITH_GIFT_LABEL'))?></span>
					</span>
					<? endif ?>
