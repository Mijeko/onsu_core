...
if (isset($arResult['REQUEST_ITEMS']))
{
	...
	<div id="<?=$injectId?>" itemprop="isRelatedTo" itemscope itemtype="https://schema.org/Product" class="sale_gift_product_container <?=$arParams['ORDER_DETAIL_BLOCKS'] ? 'drag-section sPrGiftProducts' : ''?>" <?=$arParams['ORDER_DETAIL_BLOCKS'] ? "data-order=".$arParams['ORDER_DETAIL_BLOCKS']['order-sPrGiftProducts']."" : ''?>></div>
...
?>
<div class="similar-products viewed-products gift-slider<?= $bHoverMode ? ' __expanded' : '' ?>"   id="viewed-products-gift-slider">
<?if(empty($arParams['HIDE_BLOCK_TITLE']) || $arParams['HIDE_BLOCK_TITLE'] !== 'Y'):?>
	<header><?=htmlspecialcharsBx($arParams['BLOCK_TITLE'])?></header>
<?endif?>
		...
		<div class="scroll-slider frame">
			<div class="slider-content slidee catalog"><?
				foreach($arResult['ITEMS'] as $key => $arItem):
					...
					$strObName = 'ob'.preg_replace("/[^a-zA-Z0-9_]/", "x", $strMainID).'_gift';
					$imgTitle = (
				...
				?><div class="slider-item<?=(!$arItem['CAN_BUY'] && !$arItem['ON_REQUEST'] ? ' out-of-stock' : '')?> catalog-item" id="<?=$arItemIDs['ID']?>">
					...
					<span class="stickers">
						<span class="sticker i-am-gift flaticon-6-2"><?=($arParams['TEXT_LABEL_GIFT']?:GetMessage('BITRONIC2_GIFTS_STICKER_TEXT'))?></span>
					</span>
...
(function($){
	...
	var $withGift = $stickerwrap.find('.plus-gift');
	if ($withGift.length < 1) {
		$withGift = $('<div class="gifts-stickers-wrapper"><span class="sticker-wrapper plus-gift flaticon-6" data-tooltip title="<?=GetMessage('BITRONIC2_GIFTS_TO_GIFTS')?>"><span class="text"><?=GetMessage("BITRONIC2_GIFTS_MAIN_PRODUCT_TEXT")?></span></span></div>').hide();
        $stickerwrap.before($withGift);
	}
	$withGift.slideDown();
})(jQuery);
</script>
