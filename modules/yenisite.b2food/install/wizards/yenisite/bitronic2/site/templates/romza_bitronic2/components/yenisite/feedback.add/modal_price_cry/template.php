    ...
    <div class="modal fade modal-form" id="modal_cry-for-price" tabindex="-1">
        <div class="modal-dialog">
            ...
            <form data-toggle="validator" method="post" action="#" data-rzoption="captcha-cry-for-price"
                  class="form_cry-for-price modal-form-w-icons"<? \Yenisite\Core\Ajax::printAjaxDataAttr($this) ?>>
                <? endif ?>
                <? if ($isAjax): ?>
                    <input type="hidden" name="<?= $arResult['CODE'] ?>[PRODUCT]"
                           value="<?= $_REQUEST['PRODUCT'] ? htmlspecialcharsbx($_REQUEST['PRODUCT']) : htmlspecialcharsbx($_REQUEST[$arResult['CODE']]['PRODUCT']) ?>"/>
                    <input type="hidden" name="<?= $arResult['CODE'] ?>[CURRENCY]"
                           value="<?= $_REQUEST['CURRENCY'] ? htmlspecialcharsbx($_REQUEST['CURRENCY']) : htmlspecialcharsbx($_REQUEST[$arResult['CODE']]['CURRENCY']) ?>"/>
                    <input type="hidden" name="<?= $arResult['CODE'] ?>[PRICE_TYPE_ID]"
                           value="<?= $_REQUEST['PRICE_TYPE'] ? htmlspecialcharsbx($_REQUEST['PRICE_TYPE']) : htmlspecialcharsbx($_REQUEST[$arResult['CODE']]['PRICE_TYPE_ID']) ?>"/>
                    <input type="hidden" name="<?= $arResult['CODE'] ?>[PRICE]"
                           value="<?= $_REQUEST['PRICE'] ? htmlspecialcharsbx($_REQUEST['PRICE']) : htmlspecialcharsbx($_REQUEST[$arResult['CODE']]['PRICE']) ?>"/>
                    ...
                    <div class="title-h2"><span
                                class="header-text"><?= GetMessage("RZ_POZHALOVATSYA_NA_TCENU") ?></span></div>

                    <div>
                        <div class="text-w-icon url"><?= GetMessage("RZ_VVEDITE_SSILKU_NA_ETOT_TOVAR_V_MAGAZINE") ?>
                            <span
                    ...
                    <div>
                        <div class="text-w-icon"><?= GetMessage("RZ_VASH") ?> e-mail<span
                                    class="required-asterisk">*</span>:
                        </div>
                    ...
                    <div>
                        <div class="text-w-icon"><?= GetMessage("RZ_FAMILIYA_IMYA") ?>(<?= GetMessage("RZ_OTCHESTVO") ?>
                            )<span
                                    class="required-asterisk">*</span>:
                        </div>
                    ...
                    <div>
                        ...
                        <label class="textinput-wrapper phone">
                            ...
                            <input type="text" name="<?= $arResult['CODE'] ?>[PHONE]"
                                   ...
                                   class="textinput phone-masked"
                                   value="<?= (!empty($arResult['DATA']['PHONE'])) ? $arResult['DATA']['PHONE'] : '' ?>">
                            <span class="textinput-icons">
						<i class="icon-valid flaticon-check33"></i>
						<i class="icon-not-valid flaticon-x5"></i>
					</span>
                    ...
                    <? if (!empty($arResult["CAPTCHA_CODE"])): ?>
                        <div>
                            ...
                            <div><?= GetMessage('RZ_CAPTCHA_INPUT_WORD') ?><span class="required-asterisk">*</span>:
                            </div>
                    ...
                    <? endif ?>
                    <? if ($arParams['USE_GOOGLE_CAPTCHA'] == 'Y' &&  CRZBitronic2CatalogUtils::checkGoogleCaptchaShow()): ?>
                        <div class="google-captcha">
                            <? CRZBitronic2CatalogUtils::showGoogleCaptcha() ?>
                        </div>
                    <? endif ?>
