function UmSlider(el, options){
	...
	this.updatePages = function(){
		...
		that.pages[that.activePage].addClass('active');
		that.updateDots();
		that.updateBtns();
		that.changeNumeric();
	}
	...
	this.changePage = function(targetPage){
		...
		that.onChange(that.pages[prevPage], that.pages[that.activePage], dir);
	}
